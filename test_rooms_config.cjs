// Test hotel availability with room configuration
...
// Test with room configuration
async function testRoomConfiguration(token) {
  const testCases = [
    {
      name: 'With Rooms JSON (original format)',
      params: {
        Checkin: '2025-06-26',
        Checkout: '2025-06-27',
    ...
    {
      name: 'With Adults parameter only',
      params: {
        Checkin: '2025-06-26',
        Checkout: '2025-06-27',
        Adults: '2',
    ...
    {
      name: 'Different Rooms format - array',
      params: {
        Checkin: '2025-06-26',
        Checkout: '2025-06-27',
    ...
    {
      name: 'Simple Rooms format',
      params: {
        Checkin: '2025-06-26',
        Checkout: '2025-06-27',
        Rooms: '2',  // Just the number
        Currency: 'BRL',
        IdLocation: '123'
      }
    },
    {
      name: 'Without IdLocation to test general search',
      params: {
        Checkin: '2025-06-26',
        Checkout: '2025-06-27',
        Rooms: '{"Adts":2,"ChildAge":[]}',
      ...
      if (result.status === 200) {
        try {
          ...
          if (jsonData.Success && jsonData.Data && jsonData.Data.length > 0) {
            console.log('🎉 FOUND HOTELS! First hotel:', {
              name: jsonData.Data[0].HotelName || jsonData.Data[0].Name,
              city: jsonData.Data[0].Address?.City || jsonData.Data[0].City
            });
            return jsonData; // Return on actual success with data
          }
        } catch (parseError) {
        ...
        if (result.status === 500) {
          console.log(`Error preview: ${result.data.substring(0, 200)}...`);
        } else {
...
async function main() {
  try {
    console.log('🚀 Testing hotel API with room configuration...\n');
    
    const token = await getAuthToken();
    await testRoomConfiguration(token);
