// Script para investigar onde o voo específico está sendo vendido
  ...
  try {
    ...
    if (response1.data && response1.data.Data && response1.data.Data.length > 0) {
      ...
      const targetFlight = response1.data.Data.find(flight => {
        ...
        return numeroVoo && numeroVoo.includes('9058') && preco > 500; // Ajustado para capturar diferentes preços
      });
      ...
      if (targetFlight) {
        ...
        // 5. Análise de campos específicos que podem indicar origem
        const camposRelevantes = ['isReservaFacil', 'consolidator', 'thirdParty', 'external'];
        camposRelevantes.forEach(campo => {
          if (targetFlight[campo]) {
            fontesCandidatas.push(`${campo}: ${targetFlight[campo]}`);
          }
        });
        ...
        if (fontesCandidatas.length > 0) {
          ...
          console.log('\n🛒 ONDE PROCURAR:');
          // Determina onde buscar baseado nas fontes encontradas
          if (fontesCandidatas.some(f => f.toLowerCase().includes('decolar'))) {
            console.log('   📍 Decolar.com - Busque pelo voo LA-9058');
          } else if (fontesCandidatas.some(f => f.toLowerCase().includes('viajanet'))) {
            console.log('   📍 ViajaNet.com.br - Procure pelo voo LA-9058');
          } else if (fontesCandidatas.some(f => f.toLowerCase().includes('latam'))) {
            console.log('   📍 Site oficial da LATAM - latam.com');
          } else {
            console.log('   📍 Site oficial da LATAM ou consolidadoras (Decolar, ViajaNet)');
          }
          
        } else {
          console.log('\n❌ Nenhuma fonte específica identificada nos dados');
          console.log('\n📍 RECOMENDAÇÃO: Procure no site oficial da LATAM ou em:');
          console.log('   - Decolar.com');
          console.log('   - ViajaNet.com.br');
          console.log('   - 123milhas.com');
        }
        ...
        console.log('❌ Voo LA-9058 não encontrado');
        console.log('\n📋 Voos disponíveis:');
        response1.data.Data.slice(0, 10).forEach((flight, index) => {
          const preco = flight.priceWithTax || flight.price || flight.totalPrice || 0;
          const voo = flight.numeroVoo || flight.FlightNumber || 'N/A';
          const companhia = flight.companhia || 'N/A';
          console.log(`   ${index + 1}. ${voo} (${companhia}) - R$ ${preco.toFixed(2)}`);
        });
      }
    } else {
